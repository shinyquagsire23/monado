...
DEBUG_GET_ONCE_LOG_OPTION(ahardwarebuffer_log,
                          "AHARDWAREBUFFER_LOG",
                          U_LOGGING_WARN)
#define AHB_TRACE(...)                                                         \
	U_LOG_IFL_T(debug_get_log_option_ahardwarebuffer_log(), __VA_ARGS__)
#define AHB_DEBUG(...)                                                         \
	U_LOG_IFL_D(debug_get_log_option_ahardwarebuffer_log(), __VA_ARGS__)
#define AHB_INFO(...)                                                          \
	U_LOG_IFL_I(debug_get_log_option_ahardwarebuffer_log(), __VA_ARGS__)
#define AHB_WARN(...)                                                          \
	U_LOG_IFL_W(debug_get_log_option_ahardwarebuffer_log(), __VA_ARGS__)
#define AHB_ERROR(...)                                                         \
	U_LOG_IFL_E(debug_get_log_option_ahardwarebuffer_log(), __VA_ARGS__)
...
vk_format_to_ahardwarebuffer(uint64_t format)
{
	...
	case 127 /*VK_FORMAT_S8_UINT*/: return AHARDWAREBUFFER_FORMAT_S8_UINT;
	case 126 /*VK_FORMAT_D32_SFLOAT*/:
		return AHARDWAREBUFFER_FORMAT_D32_FLOAT;
	case 97 /*VK_FORMAT_R16G16B16A16_SFLOAT*/:
		return AHARDWAREBUFFER_FORMAT_R16G16B16A16_FLOAT;
	case 43 /*VK_FORMAT_R8G8B8A8_SRGB*/:
		return AHARDWAREBUFFER_FORMAT_R8G8B8A8_UNORM /* apply
                                ...
                                struct xrt_image_native *out_images)
{
	...
	for (size_t i = 0; i < num_images; ++i) {
		int ret =
		    AHardwareBuffer_allocate(&desc, &(out_images[i].handle));
		if (ret != 0) {
...
android_ahardwarebuffer_allocator_create()
{
	struct xrt_image_native_allocator *xina =
	    U_TYPED_CALLOC(struct xrt_image_native_allocator);
