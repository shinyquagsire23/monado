...
#ifndef XR_USE_PLATFORM_SDL
#error "Must build this file with SDL2 enabled!"
#endif
                             ...
                             struct oxr_session *sess)
{
    ...
    struct xrt_compositor_gl *xcgl = xrt_gfx_provider_create_gl_sdl(
        xcn, next->sdlWindow, next->sdlContext);

    if (xcgl == NULL) {
        return oxr_error(log, XR_ERROR_INITIALIZATION_FAILED, "Failed to create an sdl client compositor");
    }
